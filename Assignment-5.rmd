...
detailing_df %>%
      group_by(type) %>%
      ##TODO- Do we also need to group by physician id here?
      summarise(
                  mean = mean(det),
                  sd = sd(det),
                  median= median(det)
...
Response : There is a slight improvement in the elasticity of detailing stock suggesting that the previous regression contained higher error in the coefficient because. Also, detailing has an effect on choice in more than its current time period.
...
gamma = 0.024585 ##beta_1
delta = 0.019629 / gamma ##beta_2 = gamma * delta
print (paste('Gamma', gamma))
print (paste('Delta', delta))
print (paste('Depreciation', 1 - delta))
```


Interpret the estimate of $\gamma$ and the estimated carry-over.
The detailing stock elasticity (gamma) is positive, suggesting that detailing has an impact on prescription of Cialis. The Delta, too is high, indicating that detailing has prescription over more than one time period, with little depreciation in the latter period.
...
```{r}
ggplot(as.data.frame(Pr_0), aes(x = Pr_0)) + ...
```
